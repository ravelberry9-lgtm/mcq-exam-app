...
# Auto-decompress content.db.gz → content.db when running on Railway
# (the repo ships only the compressed file to save space)
if not DATA_DB.exists() and DATA_DB_GZ.exists():
    print(f"Decompressing {DATA_DB_GZ.name} …")
    with gzip.open(str(DATA_DB_GZ), "rb") as f_in, open(str(DATA_DB), "wb") as f_out:
        shutil.copyfileobj(f_in, f_out)
    print(f"  → content.db ({DATA_DB.stat().st_size:,} bytes)")
...
def main():
    if not DATA_DB.exists():
        print(f"ERROR: content DHnot found at {DATA_DB}")
        sys.exit(1)
