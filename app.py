...
def _seed_ch2_mcqs_inner():
    ...
    # Ensure chapter_mcqs table exists (both backends)
    if USE_POSTGRES:
    ...
    row = db_exec(conn, "SELECT id FROM study_notes WHERE topic='Indian_History' AND chapter_num=2", fetch='one')
    if not row:
        conn.close()
    ...
    mcqs = [
        ...
        (1,'easy',
         ...
         'C',
         'డెన్మార్క్ పురావస్తు శాస్త్రవేత్త C.J.థామ్‌సన్ శిలాయుగాన్ని Palaeolithic, Mesolithic, Neolithic అని మూడు భాగాలుగా విభజించారు.'),
        ...
        (5,'hard',
         'ఆంధ్రప్రదేశ్‌లోని పాత్న (MH) వద్ద పాలియోలిథిక్ కాలానికి చెందిన ఏ అద్భుతమైన ఆధారం కనుగొనబడింది?',
         'బంగారు ఆభరణాలు','నిడుపాటి శవకోష్టిక','ఆస్ట్రిచ్ గుడ్డు పెంకులు','మట్టి ముద్రలు',
         'C',
         'ఆంధ్రప్రదేశ్‌లోని పాత్న (MH) వద్ద ఆస్ట్రిచ్ (నిప్పుకోడి) గుడ్డు పెంకులు కనుగొనబడ్డాయి — ఇవి అత్యంత పురాతన ఆధారాలు.'),
